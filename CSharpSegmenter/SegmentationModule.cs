...
namespace CSharpSegmenter
{
    class SegmentationModule
    {
        ...
        private int N;// default size
        private TiffImage image;
        private float currentThreshold; //default value
        ...
        public SegmentationModule(TiffImage image, int N, float threshold)
        {
            if (N > 0)
            {
                this.N = N;
            ...
            this.pixelArray = CreatePixelArray(this.N);
        ...
        {
            int currentPosition = (int)(Math.Pow(2, N) * coord.x + coord.y);
        ...
        private HashSet<Segment> FindBestNeighbours(Segment inputSegment)
        {
            ...
            if (setOfNeighbours.Count != 0)
            {
                ...
                {
                    float currentMergeCost = SegmentModule.MergeCost(inputSegment, neighbourSeg);
        ...
        private float FindBestMergeCost(HashSet<Segment> segmentSet, Segment segment)
        {
            ...
            if (segmentSet.Count != 0)
            {
                ...
                foreach (Segment neighbourSeg in segmentSet)
                {
                    if (firstPosition)
                    {
                        bestMergeCost = SegmentModule.MergeCost(segment, neighbourSeg);
                        firstPosition = false;
                    }
                    float currentMergeCost = SegmentModule.MergeCost(segment, neighbourSeg);
        ...
        private List<Coordinate> FindListNeighbourCoords(Segment segment)
        {
            ...
            {
                int currentXCoord = coord.x;
                int currentYCoord = coord.y;
        ...
        private List<Coordinate> FilterNeighboursCoords(Segment segment)
        {
            ...
            foreach (Coordinate coord in neighbourCoord)
            {
                int currentXCoord = coord.x;
                int currentYCoord = coord.y;
                if ((currentXCoord >= 0) && (currentYCoord >= 0) && (currentXCoord < Math.Pow(2, N)) && (currentYCoord < Math.Pow(2, N)))
                {
        ...
        ///  To find the pixel array position x * Math.Pow(2,size) + y
        /// </summary>
